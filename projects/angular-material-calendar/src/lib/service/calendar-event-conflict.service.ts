...
export class CalendarEventConflictService {
	 ...
	 */
	private conflictSearch(root: EventNode,
		i: CalendarEventFull,
		conflictingEvents: CalendarEventFull[]): CalendarEventFull {
	...
	conflicting(events: CalendarEventFull[], days: number) {
		...
		for (let i = 1; i < events.length; i++) {
			const conflictingEvents: CalendarEventFull[] = [];
